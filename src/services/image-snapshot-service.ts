...
export default class ImageSnapshotService extends PercyClientService {
  ...
  // like `ResponseService#makeLocalCopy` but works without a response object
  makeLocalCopy(imagePath: string) {
    logger.debug(`Making local copy of response: ${imagePath}`)
  ...
  // uses `PercyClient` to create resource objects for an image
  ...
  // like `SnapshotService#create` but doesn't rely on `ResourceService` which
  // creates a duplicate `PercyClient` instance unnecessarily
  ...
  // uses configuration provided during instantiation to generate snapshots for
